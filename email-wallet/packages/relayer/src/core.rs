...
pub(crate) async fn handle_email<P: EmailsPool>(
    ...
    warn!(LOG, "From address: {}", from_address; "func" => function_name!());
    check_and_update_dkim(&email, &parsed_email, &chain_client).await?;
    if is_reply_mail(&email) {
        warn!(LOG, "Reply email"; "func" => function_name!());
        ...
        if !db.contains_user(&from_address).await? {
            warn!(LOG, "Account transport"; "func" => function_name!());
            ...
            warn!(LOG, "Account init"; "func" => function_name!());
            handle_account_init(
                email,
                &parsed_email,
                account_key,
        ...
        warn!(LOG, "Non-reply email"; "func" => function_name!());
            ...
            extract_account_key_from_subject(&parsed_email.get_subject_all()?)
        {
            ...
            let wallet_salt = account_key.to_wallet_salt()?;
            warn!(LOG, "Wallet salt: {}", field2hex(&wallet_salt.0); "func" => function_name!());
            let wallet_addr = chain_client
                .get_wallet_addr_from_salt(&wallet_salt.0)
                .await?;
            warn!(LOG, "Sender wallet address: {}", wallet_addr; "func" => function_name!());
            ...
            let is_faucet = if current_count < *ONBOARDING_TOKEN_DISTRIBUTION_LIMIT.get().unwrap() {
                match chain_client.transfer_onboarding_tokens(wallet_addr).await {
                    Ok(tx_hash) => {
                        warn!(LOG, "onboarding tokens sent {:?}", tx_hash; "func" => function_name!());
                    }
                    _ => {
                        ONBOARDING_COUNTER.fetch_sub(1, Ordering::SeqCst);
                        warn!(LOG, "onboarding tokens transfer failed"; "func" => function_name!());
                    }
                }
                true
            } else {
                ONBOARDING_COUNTER.fetch_sub(1, Ordering::SeqCst);
                warn!(LOG, "onboarding token limit reached"; "func" => function_name!());
                false
            };
        ...
        warn!(LOG, "Subject: {}", subject; "func" => function_name!());
        let command = subject_templates::extract_command_from_subject(&subject)?;
        warn!(LOG, "Command: {}", command; "func" => function_name!());
        let account_key = db
        ...
        warn!(LOG, "Fee token name: {}", fee_token_name; "func" => function_name!());
            ...
            SEND_COMMAND => {
                ...
                } = &template_vals[1]
                {
                    if *is_email {
                        ...
                        warn!(LOG, "derived commit {:?}", email_addr_commit; "func" => function_name!());
                    } else {
                ...
                if let TemplateValue::TokenAmount { token_name, amount } = &template_vals[0] {
                    warn!(LOG, "token name: {}", token_name; "func" => function_name!());
                    warn!(
                        LOG,
                        "token addr: {}",
                    ...
                    warn!(LOG, "decimal size: {}", decimal_size; "func" => function_name!());
                    WalletParams {
        ...
        warn!(LOG, "parameter constructed"; "func" => function_name!());
        ...
        warn!(LOG, "masked_subject {}", masked_subject; "func" => function_name!());
        warn!(
            LOG,
        ...
        warn!(LOG, "email_op constructed: {:?}", email_op; "func" => function_name!());
        ...
        warn!(LOG, "email_op broadcased to chain: {}", tx_hash; "func" => function_name!());
        if let Some(email_addr) = recipient_email_addr.as_ref() {
            warn!(LOG, "recipient email address: {}", email_addr; "func" => function_name!());
        ...
        (*tx_sender)
            ...
            .unwrap();
        warn!(LOG, "email_op sent to tx_sender"; "func" => function_name!());
    }
    Ok(())
}
...
pub(crate) async fn handle_account_init(
    ...
    let input = generate_account_init_input(
        ...
        .await?;
    warn!(LOG, "account init input {}", input; "func" => function_name!());
    let (proof, pub_signals) =
    ...
    warn!(LOG, "account init data {:?}", data; "func" => function_name!());
    let result = chain_client.init_account(data).await?;
    warn!(LOG, "account init tx hash: {}", result; "func" => function_name!());
...
pub(crate) async fn generate_proof(
    ...
    warn!(LOG, "prover input {}", input; "func" => function_name!());
    let res = client
...
pub(crate) async fn check_and_update_dkim(
    ...
    warn!(LOG, "public_key_hash {:?}", public_key_hash; "func" => function_name!());
    let domain = parsed_email.get_email_domain()?;
    warn!(LOG, "domain {:?}", domain; "func" => function_name!());
    if chain_client
        .check_if_dkim_public_key_hash_valid(domain.clone(), fr_to_bytes32(&public_key_hash)?)
        .await?
    {
        warn!(LOG, "public key registered"; "func" => function_name!());
        return Ok(());
    }
    ...
    warn!(LOG, "selector {}", selector; "func" => function_name!());
    ...
    warn!(LOG, "DKIM oracle result {:?}", oracle_result; "func" => function_name!());
    let public_key_hash = hex::decode(&oracle_result.public_key_hash[2..])?;
    warn!(LOG, "public_key_hash from oracle {:?}", public_key_hash; "func" => function_name!());
    let signature = Bytes::from_hex(&oracle_result.signature[2..])?;
    warn!(LOG, "signature {:?}", signature; "func" => function_name!());
    let tx_hash = chain_client
        ...
        .await?;
    warn!(LOG, "DKIM registry updated {:?}", tx_hash; "func" => function_name!());
    Ok(())
}
